...
void ARelicRunnersGameMode::PostLogin(APlayerController* NewPlayer)
{
    ...
    {
        // If controller already has a preview in this world, don't spawn.
        if (IsValid(RRPC->PlayerPreviewInstance) &&
            RRPC->PlayerPreviewInstance->GetWorld() == GetWorld())
        {
            UE_LOG(LogTemp, Verbose, TEXT("[GameMode] %s already has preview, skipping spawn."), *RRPC->GetName());
        }
        else
        {
