...
void ASpawnPoint::SpawnEnemies()
{
	if ((m_pSpawnedEnemies.Num() == 0 && !FinishedAllWaves()) || m_currentAllowance > 0)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Tryed to Error"));
			if (IsAtMaxCapacity())
			{
				return;
			}
			...
			APawn* newEnemy = static_cast<APawn*>(GetWorld()->SpawnActor<APawn>(m_pPickableCards[pickedEnemyIndex]->m_enemyType, GetRandomPosition() + FVector(0, 40, 0), FRotator::ZeroRotator));
			...
			{
				m_pPickableCards.RemoveAt(pickedEnemyIndex);
				continue;
			}
		}
	}
